...
def format_row4_as_date(ws, num_cols):
    """
    Safely format row 4 from column D to the last column with data as date dd-mm-yyyy.
    ...
    try:
        ...
        if num_cols <= 3:  
            print("⚠️ Skipped: Not enough columns to format (need >= 4).")
            return
        ...
        print(f"❌ Failed to format row 4: {e}")
